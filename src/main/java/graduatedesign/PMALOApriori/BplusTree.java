...
public class BplusTree {
    ...
    static class Node {
        ...
        public Node(boolean isLeaf) {
            this.isLeaf = isLeaf;
            entries = new ArrayList<Entry<String, Object>>();
        ...
        public void insertOrUpdate(String key, Object obj, BplusTree tree) {
            //如果是叶子节点
            if (isLeaf) {
                //不需要分裂，直接插入或更新
                ...
                if (isRoot) {
                    ...
                    if (entries.size() > tree.getOrder() / 2 && entries.size() > 2) {
                        ...
                        if (previous != null
                            ...
                            Entry<String, Object> entry = previous.getEntries().get(size - 1);
                            ...
                            Entry<String, Object> entry = next.getEntries().get(0);
         ...
         * 插入到当前节点的关键字中
         */
        protected void insertOrUpdate(String key, Object obj) {
            Entry<String, Object> entry = new SimpleEntry<String, Object>(key, obj);
            //如果关键字列表长度为0，则直接插入
            if (entries.size() == 0) {
                entries.add(entry);
                return;
        ...
        public List<Entry<String, Object>> getEntries() {
            return entries;
        }

        public void setEntries(List<Entry<String, Object>> entries) {
    ...
    public static void main(String[] args) {
        ...
        for (int j = 0; j < 100000; j++) {
            for (int i = 0; i < 100; i++) {
                ...
                tree.insertOrUpdate(String.valueOf(randomNumber), randomNumber);
            }

            for (int i = 0; i < 100; i++) {
                int randomNumber = random.nextInt(1000);
                tree.remove(String.valueOf(randomNumber));
            }
        }
        ...
        tree.insertOrUpdate("abc", 123.5641654);

        int search = 100;
        System.out.println(tree.get("abc"));
        System.out.println(tree.get(String.valueOf(search)));
    }
}